...
public class LoadCheckoutData extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("loadcheckout");
...
//                address.getUser().setMobile(null);
            ...
            if (cartList.isEmpty()) {
                responseObject.addProperty("message", "Empty cart");
                  ...
                  // Clean sensitive user info and avoid invalid calls
                for (Cart cart : cartList) {
                    cart.setUser(null); // remove user from cart to avoid exposing sensitive info

                    Product product = cart.getProduct();
                    if (product != null) {
                        // Remove nested objects or user references if needed
                        product.setAdmin(null);
                        product.setCategory(null);
                        product.setColor(null);
                        product.setModel(null);
                        product.setStatus(null);
                        // IMPORTANT: Do NOT call product.setUser(null) as Product has no user field
                    }
                }
